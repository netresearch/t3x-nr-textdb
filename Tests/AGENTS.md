...
- **Unit/**: Isolated unit tests (no TYPO3 bootstrap) — Controller, Domain/Model, Service, ViewHelpers
- **Functional/**: Integration tests with database — Controller, Domain/Repository, Service, ViewHelpers; shared base class `Functional/AbstractFunctionalTestCase.php`; fixtures in `Functional/Fixtures/`

## Setup & environment

**Prerequisites:**
- Same as Classes/AGENTS.md (PHP 8.2+, TYPO3 14.3+)
- Dev tooling (PHPUnit, typo3/testing-framework, PHPStan, Infection, …) comes via the `netresearch/typo3-ci-workflows` meta-package (the only `require-dev` entry)

**Installation:**
```bash
composer install  # Installs dev tooling to .Build/vendor
```

**Test configuration:**
- Unit config: `Build/phpunit.xml` (PHPUnit 13)
- Functional config: `Build/FunctionalTests.xml` (SQLite via `typo3DatabaseDriver=pdo_sqlite`)
- Mutation config: `infection.json5`
...
.Build/bin/phpunit -c Build/phpunit.xml Tests/Unit/Domain/Model/SomeTest.php

# Run without coverage (faster)
.Build/bin/phpunit -c Build/phpunit.xml --no-coverage

# Verbose output
.Build/bin/phpunit -c Build/phpunit.xml --testdox
```

**Test structure:**
- Unit tests: `Tests/Unit/` mirrors `Classes/` structure
- Functional tests: `Tests/Functional/`, extend `AbstractFunctionalTestCase`
- Example: `Classes/Domain/Model/Foo.php` → `Tests/Unit/Domain/Model/FooTest.php`

## Code style & conventions

**PHPUnit patterns:**
- Extend `TYPO3\TestingFramework\Core\Unit\UnitTestCase` for unit tests, `AbstractFunctionalTestCase` for functional tests
- Use PHPUnit attributes: `#[Test]` on test methods, `#[CoversClass(...)]`/`#[UsesClass(...)]` on the class (no `@test` annotations — PHPUnit 13)
- Use `setUp()` and `tearDown()` for test fixtures
- Use data providers (`#[DataProvider('providerName')]`, static provider method) for multiple test cases
...
- [ ] Extends appropriate base class (`UnitTestCase` or `AbstractFunctionalTestCase`)
...
- Official docs: https://docs.phpunit.de/ (this project uses PHPUnit 13, see the schema in `Build/phpunit.xml`)

**Project-specific:**
- Check existing test patterns in `Tests/Unit/` and `Tests/Functional/`
- Review `Build/phpunit.xml` and `Build/FunctionalTests.xml` for configuration
- Look at TYPO3 core tests for complex patterns

**Debugging tests:**
```bash
# Run with verbose output
.Build/bin/phpunit -c Build/phpunit.xml --testdox --colors

# Run single test method
.Build/bin/phpunit -c Build/phpunit.xml --filter testMethodName
...
- Use `AbstractFunctionalTestCase` (extends TYPO3's `FunctionalTestCase`) for integration tests with DB
